...
import { Scissors, Calendar, User, Clock, Phone, Lock, LogOut, CheckCircle, XCircle, Edit2, Trash2, Plus, Settings, Check, Type } from 'lucide-react';
...
export default function App() {
  ...
  // Dükkan Ayarları (Admin değiştirebilir)
  const [shopSettings, setShopSettings] = useState({
    shopName: "BarberShop",
    shopNameHighlight: "Shop", // İsmin renkli kısmı
    mainTitle: "Tarzınızı",
  ...
  // Berber Hizmetleri ve Fiyatları
  const [services, setServices] = useState([
    ...
    { id: 5, name: "Çocuk Tıraşı", price: 200 },
  ]);
  ...
  const [adminTab, setAdminTab] = useState('appointments'); // appointments, services, settings
  const [editingService, setEditingService] = useState(null);
  
  // Dükkan Ayarlarını Kaydetme Formu İçin Geçici State
  ...
  const handleSaveSettings = (e) => {
    e.preventDefault();
    setShopSettings(editingSettings);
    alert("Dükkan ayarları başarıyla güncellendi!");
  };
      ...
      <div className="min-h-screen bg-neutral-900 text-neutral-100 font-sans selection:bg-amber-500 selection:text-neutral-900">
        ...
        <header className="border-b border-neutral-800 bg-neutral-950/50 sticky top-0 z-10 backdrop-blur-md">
          <div className="max-w-5xl mx-auto px-4 py-4 flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              {/* PANEL'DEN DEĞİŞEN DÜKKAN İSMİ BURADA */}
            ...
            <button 
              ...
              className="text-neutral-500 hover:text-amber-500 transition-colors flex items-center gap-1 text-xs uppercase tracking-widest"
        ...
        <main className="max-w-5xl mx-auto px-4 py-12">
          <div className="text-center mb-12">
            {/* PANEL'DEN DEĞİŞEN BAŞLIK VE AÇIKLAMA BURADA */}
            ...
            {services.map((service) => (
              <div key={service.id} className="bg-neutral-800/50 border border-neutral-700 rounded-2xl p-6 hover:border-amber-500/50 transition-all group">
                ...
                <button 
                  ...
                  className="w-full bg-neutral-700 hover:bg-amber-500 hover:text-neutral-900 text-white font-bold py-3 px-4 rounded-xl transition-all flex justify-center items-center gap-2"
                >
                  <Calendar size={18} />
                  Randevu Al
                </button>
              ...
              {isSuccess ? (
                <div className="p-8 text-center">
                  ...
                  <p className="text-neutral-400 mb-6">
                    {formData.date} günü saat {formData.time} için talebiniz bize ulaştı. Sizi bekliyoruz!
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <form onSubmit={handleBook} className="p-6 space-y-4">
                     <div>
                      ...
                      <div className="relative">
                        ...
                        <input 
                          type="text" required
                          className="w-full bg-neutral-950 border border-neutral-800 text-white pl-10 pr-4 py-3 rounded-xl focus:ring-1 focus:ring-amber-500 focus:border-amber-500 outline-none"
                          value={formData.name}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input 
                            type="date" required
                            className="w-full bg-neutral-950 border border-neutral-800 text-white pl-10 pr-4 py-3 rounded-xl focus:ring-1 focus:ring-amber-500 focus:border-amber-500 outline-none [color-scheme:dark]"
                            value={formData.date}
      ...
      <div className="min-h-screen bg-neutral-950 flex items-center justify-center p-4 font-sans">
        <div className="bg-neutral-900 p-8 rounded-3xl shadow-2xl w-full max-w-sm border border-neutral-800">
          <div className="text-center mb-8">
            <div className="bg-neutral-800 w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-neutral-700">
          ...
          <form onSubmit={handleLogin}>
            <div className="mb-6">
              <input 
                name="password" type="password" placeholder="Şifre (1234)"
                className="w-full px-4 py-4 bg-neutral-950 border border-neutral-800 text-white rounded-xl focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none text-center tracking-widest text-lg" required
              />
              {loginError && <p className="text-red-400 text-sm mt-3 text-center">{loginError}</p>}
            </div>
            <button type="submit" className="w-full bg-amber-500 hover:bg-amber-400 text-neutral-950 font-bold py-3.5 rounded-xl transition-colors">
              Giriş Yap
            </button>
          </form>
          
          <button onClick={() => setView('customer')} className="w-full mt-4 text-neutral-500 hover:text-white text-sm py-2">
  ...
  // ==========================================
  // 3. YÖNETİCİ PANELİ (YENİ AYARLAR SEKMESİ EKLENDİ)
  // ==========================================
  if (view === 'admin_panel') {
      ...
      <div className="min-h-screen bg-neutral-100 flex flex-col md:flex-row font-sans">
        ...
        <div className="w-full md:w-64 bg-neutral-900 text-white flex flex-col border-r border-neutral-800">
          <div className="p-6 border-b border-neutral-800">
            <div className="flex items-center gap-2 text-amber-500 font-bold text-xl mb-1">
              <Scissors size={20} className="-rotate-45" /> {shopSettings.shopName}
            </div>
          </div>
          ...
          <nav className="flex-1 p-4 space-y-2">
            ...
            {/* YENİ SEKMEYİ BURAYA EKLEDİK */}
            <button onClick={() => setAdminTab('shop_settings')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${adminTab === 'shop_settings' ? 'bg-amber-500 text-neutral-900 font-bold' : 'hover:bg-neutral-800 text-neutral-300'}`}>
              <Type size={18} /> Dükkan Ayarları
            </button>
          </nav>
          
          <div className="p-4 border-t border-neutral-800">
            <button onClick={() => setView('customer')} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-white transition-colors text-sm">
              <LogOut size={16} /> Dükkana Geri Dön
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 p-4 md:p-8 overflow-y-auto bg-neutral-100 text-neutral-900">
                    ...
                    <div key={app.id} className="bg-white rounded-2xl p-5 border border-neutral-200 shadow-sm flex flex-col md:flex-row justify-between md:items-center gap-4">
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className={`px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wider ${
                            app.status === 'Bekliyor' ? 'bg-amber-100 text-amber-700' : 
                            app.status === 'Onaylandı' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                          }`}>{app.status}</span>
                        </div>
                      ...
                      {app.status === 'Bekliyor' && (
                        <div className="flex gap-2 border-t md:border-t-0 border-neutral-100 pt-4 md:pt-0">
                          <button onClick={() => handleUpdateStatus(app.id, 'Onaylandı')} className="flex-1 md:flex-none flex items-center justify-center gap-1 bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-xl font-medium text-sm transition-colors">
                            <Check size={16} /> Onayla
                          </button>
                          <button onClick={() => handleUpdateStatus(app.id, 'İptal')} className="flex-1 md:flex-none flex items-center justify-center gap-1 bg-red-100 hover:bg-red-200 text-red-600 px-4 py-2 rounded-xl font-medium text-sm transition-colors">
          ...
          {/* SEKME 2: FİYAT VE HİZMET YÖNETİMİ (Öncekiyle aynı) */}
          {adminTab === 'services' && (
             <div className="max-w-4xl mx-auto">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-black text-neutral-800 flex items-center gap-2">
                  <Settings className="text-amber-500" /> Fiyat Listesi Yönetimi
                </h2>
                <button onClick={handleAddService} className="bg-neutral-900 hover:bg-neutral-800 text-white px-4 py-2 rounded-xl font-medium flex items-center gap-2 transition-colors text-sm">
          ...
          {/* SEKME 3: YENİ - DÜKKAN AYARLARI */}
          {adminTab === 'shop_settings' && (
            <div className="max-w-3xl mx-auto">
              <h2 className="text-2xl font-black text-neutral-800 mb-6 flex items-center gap-2">
                <Type className="text-amber-500" /> Dükkan Ayarları
              </h2>
                ...
                <form onSubmit={handleSaveSettings} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-neutral-50 rounded-xl border border-neutral-100">
                    <div>
                      ...
                      <input 
                        type="text" 
                        value={editingSettings.shopName}
                        onChange={(e) => setEditingSettings({...editingSettings, shopName: e.target.value})}
                        className="w-full px-4 py-2 border border-neutral-300 rounded-lg outline-none focus:border-amber-500"
                        required
                      />
                    </div>
                    <div>
                      ...
                      <input 
                        type="text" 
                        value={editingSettings.shopNameHighlight}
                        onChange={(e) => setEditingSettings({...editingSettings, shopNameHighlight: e.target.value})}
                        className="w-full px-4 py-2 border border-neutral-300 rounded-lg outline-none focus:border-amber-500"
                      />
                      <p className="text-xs text-neutral-500 mt-1">Örn: "Shop" yazarsanız, BerberShop'taki Shop sarı olur.</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-neutral-50 rounded-xl border border-neutral-100">
                     <div>
                      ...
                      <input 
                        type="text" 
                        value={editingSettings.mainTitle}
                        onChange={(e) => setEditingSettings({...editingSettings, mainTitle: e.target.value})}
                        className="w-full px-4 py-2 border border-neutral-300 rounded-lg outline-none focus:border-amber-500"
                        required
                      />
                    </div>
                    <div>
                      ...
                      <input 
                        type="text" 
                        value={editingSettings.mainTitleHighlight}
                        onChange={(e) => setEditingSettings({...editingSettings, mainTitleHighlight: e.target.value})}
                        className="w-full px-4 py-2 border border-neutral-300 rounded-lg outline-none focus:border-amber-500"
                      />
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-neutral-50 rounded-xl border border-neutral-100">
                    ...
                    <textarea 
                      ...
                      className="w-full px-4 py-3 border border-neutral-300 rounded-lg outline-none focus:border-amber-500 min-h-[100px]"
                      required
                    ></textarea>
                  </div>

                  <div className="pt-4 border-t border-neutral-200">
                    <button type="submit" className="bg-amber-500 hover:bg-amber-400 text-neutral-900 font-bold py-3 px-8 rounded-xl transition-colors w-full md:w-auto">
